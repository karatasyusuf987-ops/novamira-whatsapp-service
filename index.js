...
const client = new Client({
  ...
  puppeteer: {
    headless: true,
    // Default 30s is too short — WhatsApp Web occasionally takes 60-90s to
    // respond to RPC calls (especially `getChats` and `downloadMedia`).
    // When it times out, the message handler hangs and incoming messages
    // pile up unprocessed → panel sees nothing, AI never fires.
...
async function buildMessagePayload(msg, fromMe) {
  ...
  if (msg.hasMedia) {
    try {
      // Race the download against a 20s wall-clock timeout. WhatsApp Web's
      // RPC can hang for the full 2-min protocolTimeout on big files —
      // we'd rather drop the media than block the event loop for everyone.
      const media = await Promise.race([
        msg.downloadMedia(),
        new Promise((_, rej) =>
          setTimeout(() => rej(new Error("media_download_timeout_20s")), 20_000),
        ),
      ]);
      if (media) {
      ...
      // Non-fatal — text body still ships to backend so AI can reply.
    }
  }
...
// ---------------------------------------------------------------------------
// Auto-healing watchdog — every 5 min, when status === "ready", call
// `client.getState()` over the live Puppeteer page. If it hangs >30s or
// throws, the WhatsApp Web tab is dead → exit with code 1 so Railway/Render
// auto-restart the container with a clean Chromium. Without this, the tab
// can silently go zombie (status still says "ready", incoming messages
// vanish, AI never fires) and only a manual restart fixes it.
// ---------------------------------------------------------------------------
...
setInterval(async () => {
  ...
  try {
    const probe = Promise.race([
      client.getState(),
      new Promise((_, rej) =>
        setTimeout(() => rej(new Error("getState_timeout_30s")), 30_000),
      ),
    ]);
    const state = await probe;
    if (state !== "CONNECTED") {
      console.warn(`[watchdog] unexpected state: ${state}`);
      consecutiveHealthFails++;
    } else {
      if (consecutiveHealthFails > 0) {
        console.log("[watchdog] recovered, state=CONNECTED");
      }
    ...
    consecutiveHealthFails++;
    console.warn(
      `[watchdog] health probe failed (${consecutiveHealthFails}/2): ${e.message}`,
    );
  }
  if (consecutiveHealthFails >= 2) {
    console.error(
      "[watchdog] WhatsApp Web tab is dead — exiting so the platform can restart us with a clean Chromium",
    );
    process.exit(1);
  }
}, 5 * 60 * 1000);
